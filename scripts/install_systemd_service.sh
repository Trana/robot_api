...
TEMPLATE_PATH="${REPO_DIR}/deploy/systemd/robot-api.service.template"
ENV_EXAMPLE_PATH="${REPO_DIR}/deploy/systemd/robot-api.env.example"

APP_DIR="${ROBOT_API_APP_DIR:-/opt/robot_api}"
SERVICE_USER="${ROBOT_API_SERVICE_USER:-$(id -un)}"
SERVICE_GROUP="${ROBOT_API_SERVICE_GROUP:-$(id -gn)}"
...
sed \
  ...
  -e "s|__ROBOT_API_USER__|${SERVICE_USER}|g" \
  -e "s|__ROBOT_API_GROUP__|${SERVICE_GROUP}|g" \
...
echo "Installed robot-api.service"
echo "Next steps:"
echo "  1) Edit ${ENV_FILE_PATH}"
echo "  2) systemctl restart robot-api"
echo "  3) systemctl status robot-api"
